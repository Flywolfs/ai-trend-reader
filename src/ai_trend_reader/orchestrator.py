...
class Orchestrator:
    def __init__(self, settings: Settings, dry_run: bool = False, source: str | None = None):
        ...
        self.source_filter = source  # "github", "arxiv", or None (both)
    ...
    async def run(self) -> DigestReport:
        ...
        try:
            ...
            github_items, arxiv_items = await self._fetch_sources()
            stats.github_fetched = len(github_items)
            stats.arxiv_fetched = len(arxiv_items)
            logger.info("fetch_complete", github=len(github_items), arxiv=len(arxiv_items))
            ...
            if not self.dry_run:
                ...
                seen_pairs = [
                    ...
                    for i in github_items + arxiv_items
                ]
                ...
                # Print report preview to stdout
    ...
    async def _fetch_sources(self) -> tuple[list[TrendItem], list[TrendItem]]:
        ...
        return github_items, arxiv_items
